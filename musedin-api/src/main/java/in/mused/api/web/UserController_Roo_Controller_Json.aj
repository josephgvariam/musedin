...
privileged aspect UserController_Roo_Controller_Json {
    ...
    @ResponseBody
    public ResponseEntity<String> UserController.showJson(@PathVariable("id") ObjectId id) {
    ...
    public ResponseEntity<String> UserController.deleteFromJson(@PathVariable("id") ObjectId id) {
        User user = securityService.findUser(id);
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        if (user == null) {
